	...
	public class FZipFile
	{
			...
			if(!centralDir && _content.length > 0) {
				if(isCompressed) {
					if(HAS_UNCOMPRESS || HAS_INFLATE) {
						...
						// Content contains zlib header and checksum. Ignore those.
		...
		internal function parseContent(data:IDataInput):void {
			if(_compressionMethod === COMPRESSION_DEFLATED && !_encrypted) {
				if(HAS_UNCOMPRESS || HAS_INFLATE) {
					// Adobe Air and FP10+ support deflate compression
					// so we can decompress without using the Adler32 hack
					// we just write out the raw deflate compressed file
					data.readBytes(_content, 0, _sizeCompressed);
				} else if(_hasAdler32) {
					// Only zlib available (FP9), but Adler32 checksum present:
